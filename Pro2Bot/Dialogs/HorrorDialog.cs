...
{
    public class HorrorDialog : ComponentDialog
    {
        public HorrorDialog() : base(nameof(HorrorDialog))
        {
        ...
        private async Task<DialogTurnResult> GetHorrorAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            using (var httpClient = new HttpClient { BaseAddress = baseAddress })
            {
                ...
                using (var response = await httpClient.GetAsync("discover/movie?api_key=265b44f5f18cefae943f3f30ec973d54&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=27"))
                {
                    string responseData = await response.Content.ReadAsStringAsync();

                    var model = JsonConvert.DeserializeObject<HorrorRootObject>(responseData);
            ...
            await stepContext.Context.SendActivityAsync(MessageFactory.Text("Here are Horror movies!"));
        ...
        public class HorrorRootObject
        {
            public int page { get; set; }
            public List<HorrorResult> results { get; set; }
