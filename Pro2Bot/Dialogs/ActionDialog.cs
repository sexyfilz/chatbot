...
namespace Pro2Bot.Dialogs
{
    ...
    {
        public ActionDialog() : base(nameof(ActionDialog))
        {
        ...
        private async Task<DialogTurnResult> GetActionAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            using (var httpClient = new HttpClient { BaseAddress = baseAddress })
            {
                ...
                using (var response = await httpClient.GetAsync("discover/movie?api_key=265b44f5f18cefae943f3f30ec973d54&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=28"))
                {
                    string responseData = await response.Content.ReadAsStringAsync();

                    var model = JsonConvert.DeserializeObject<ActionRootObject>(responseData);
            ...
            await stepContext.Context.SendActivityAsync(MessageFactory.Text("Here are action movies!"));
